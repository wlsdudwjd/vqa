"""Standalone RoboVQA demo for Apple Silicon Macs.

Same design as demo.py, but the VLM worker uses MLX (mlx-vlm) instead of
torch + bitsandbytes, since bitsandbytes' 4-bit quantization is CUDA-only
and won't run on Apple Silicon. MLX is Apple's array framework built for
Apple Silicon's unified memory / Metal GPU, and mlx-vlm ships pre-quantized
4-bit Qwen2-VL/Qwen2.5-VL checkpoints (the "mlx-community/..." repos).

NOT verified end-to-end (written/edited on Linux, no Mac available here).
The Gradio + tensorflow + subprocess-isolation parts are a direct port of
demo.py and should work unchanged; the mlx_vlm calls in run_vlm_worker()
follow the documented API as of when this was written, but mlx-vlm's API
has moved before - if load()/generate() signatures don't match, check
https://github.com/Blaizzy/mlx-vlm for the current usage and adjust.

Run with:
    .venv/bin/python demo_mac.py
...
# Pre-quantized 4-bit MLX checkpoint (community-converted from Qwen2.5-VL-7B-Instruct).
# Browse alternatives at https://huggingface.co/mlx-community?search_models=qwen2.5-vl
VLM_MODEL_ID = 'mlx-community/Qwen2.5-VL-7B-Instruct-4bit'


# =============================================================================
# VLM worker: runs in its own process, only ever imports mlx/mlx_vlm.
# =============================================================================
...
def run_vlm_worker():
  ...
  for line in sys.stdin:
    ...
    # mlx_vlm reads images from disk paths, so decode the base64 frames to
    # temp JPEGs first.
    image_paths = []
    for i, b64 in enumerate(request['images']):
      path = f'/tmp/robovqa_mlx_frame_{i}.jpg'
      with open(path, 'wb') as f:
        f.write(base64.b64decode(b64))
      image_paths.append(path)

    prompt = apply_chat_template(processor, config, question, num_images=len(image_paths))
    answer = generate(
        model, processor, prompt, image=image_paths,
        max_tokens=max_new_tokens, verbose=False,
    )
    # Depending on mlx_vlm version, generate() returns either a string or an
    # object with a `.text` attribute - handle both.
    answer = getattr(answer, 'text', answer)

    print(json.dumps({'answer': str(answer).strip()}), flush=True)
...
# =============================================================================
# RoboVQA text parsing (identical to demo.py).
# =============================================================================
...
def run_gradio_app():
  ...
  # TF is only used to read/decode tfrecords; keep it off any GPU/Metal path
  # so it can't interfere with the MLX worker subprocess.
  ...
  print('Launching VLM worker (downloads + loads the model on first run)...')
  ...
  with gr.Blocks(title='RoboVQA Demo (Mac/MLX)') as demo:
    gr.Markdown(
        '# RoboVQA 데모 (Mac/MLX)\n'
        '랜덤 에피소드를 불러온 뒤, 데이터셋의 정답 QA를 참고하거나 '
        '직접 자유롭게 질문해서 로컬 VLM(Qwen2.5-VL-7B, MLX 4bit)의 답을 확인해보세요.'
    )
